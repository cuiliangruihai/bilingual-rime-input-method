...
const allDisplayClasses = ['display-bilingual', 'display-chinese', 'display-english']
...
function setCandidateDisplay(display: STYLE_JSON['Typography']['CandidateDisplay'] | undefined) {
  const selected = display === 'Chinese only'
    ? 'display-chinese'
    : display === 'English only'
      ? 'display-english'
      : 'display-bilingual'
  for (const c of allDisplayClasses) {
    theme.classList.toggle(`fcitx-${c}`, c === selected)
  }
}
...
export function setStyle(style: string) {
  ...
  // Typography
  // Older saved themes may not contain CandidateDisplay; keep bilingual as
  // the backwards-compatible default in that case.
  setCandidateDisplay(j.Typography.CandidateDisplay)
