...
import { auxDown, auxUp, bilingualComment, hoverables, panel, preedit, scrollArea, tabs, theme } from './selector'
...
export function setCandidates(cands: Candidate[], highlighted: number, pageable: boolean, hasPrev: boolean, hasNext: boolean, scrollState: SCROLL_STATE, scrollStart: boolean, scrollEnd: boolean, tabActions: CandidateAction[]) {
  bilingualComment.textContent = highlighted >= 0 ? (cands[highlighted]?.comment ?? '') : ''
  if (cands.length) {
  ...
  hoverables.classList.toggle('fcitx-option-output', cands.some(c => c.label.startsWith('⌥')))
  resetMouseMoveState()
  hideContextmenu()
  ...
  for (let i = 0; i < cands.length; ++i) {
    ...
    // Always create the second row.  The bilingual card theme relies on an
    // empty comment row keeping the same height when a translation is absent.
