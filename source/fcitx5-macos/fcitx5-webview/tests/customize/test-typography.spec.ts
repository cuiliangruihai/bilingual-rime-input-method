...
test('Candidate display mode toggles between bilingual, Chinese-only, and English-only', async ({ page }) => {
  await init(page)
  await setCandidates(page, [
    { text: '你好', label: '1', comment: 'hello' },
  ], 0)

  const root = page.locator('#fcitx-theme')
  await expect(root).toHaveClass(/fcitx-display-bilingual/)

  await setStyle(page, { Typography: { CandidateDisplay: 'Chinese only' } })
  await expect(root).toHaveClass(/fcitx-display-chinese/)
  await expect(root).not.toHaveClass(/fcitx-display-english/)

  await setStyle(page, { Typography: { CandidateDisplay: 'English only' } })
  await expect(root).toHaveClass(/fcitx-display-english/)
  await expect(root).not.toHaveClass(/fcitx-display-chinese/)
})
