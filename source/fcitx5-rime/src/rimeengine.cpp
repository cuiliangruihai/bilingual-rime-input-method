...
RimeEngine::RimeEngine(Instance *instance)
    ...
    if constexpr (isAndroid() || isApple()) {
        ...
        if (!defaultYaml.empty()) {
            ...
            // The macOS installer ships the Rime engine separately from the
            // data package. Reuse the user's Rime data when no shared
            // rime-data package is present, which also preserves Squirrel's
            // existing schemas and dictionaries.
            const auto userRimeDir =
                sp.userDirectory(StandardPathsType::PkgData) / "rime";
            if (std::filesystem::exists(userRimeDir / "default.yaml")) {
                sharedDataDir_ = userRimeDir.string();
            } else if (std::filesystem::exists(
                           std::filesystem::path(RIME_DATA_DIR) /
                           "default.yaml")) {
...
void RimeEngine::toggleShowEnglish(InputContext *inputContext) {
    config_.showEnglish.setValue(!*config_.showEnglish);
    safeSaveAsIni(config_, "conf/rime.conf");
    if (auto *state = this->state(inputContext)) {
        state->updateUI(inputContext, false);
    }
}
...
void RimeEngine::keyEvent(const InputMethodEntry &entry, KeyEvent &event) {
    ...
    const bool macEnglishShortcut =
        (event.key().sym() == FcitxKey_E ||
         event.key().sym() == FcitxKey_e) &&
        states.test(KeyState::Ctrl) && states.test(KeyState::Shift);
    if (event.key().checkKeyList(*config_.toggleEnglishOutput) ||
        macEnglishShortcut) {
        if (!event.isRelease()) {
            toggleShowEnglish(inputContext);
        }
